      ...
      {projectQuery.data && (
        <div className="mt-6 space-y-6">
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:bg-white/5 dark:border-white/15">
            <h3 className="text-sm font-semibold text-surface-dark dark:text-white">Project metrics</h3>
              ...
              {projectQuery.data.repos.map((r) => (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="text-xs text-surface-dark/70 dark:text-white/70">{r.activity_status ?? '—'}</div>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:bg-white/5 dark:border-white/15">
            ...
            <div className="mt-3 text-sm text-surface-dark/70 dark:text-white/70">
            ...
            <div className="mt-3 space-y-2">
              {projectQuery.data.contributors.map((c) => (
                <Link
                  ...
                  className="block rounded-xl border border-gray-200 bg-white px-4 py-3 hover:-translate-y-0.5 transition-transform dark:bg-white/5 dark:border-white/15"
                >
                  <div className="text-sm font-medium text-surface-dark dark:text-white">{c.name}</div>
                </Link>
              ))}
            </div>
          </section>
