...
import { useProjectContributorsSuspense } from "../../../lib/api/queries/projects";
...
/**
 * Top contributors aggregated across a project's repos, sourced from
 * `getProjectContributors`. Shows each contributor's total commit count
 * across every repo belonging to the project.
 */
...
function ContributorsPanelContent({ canonicalId }: { canonicalId: string }) {
  const { data } = useProjectContributorsSuspense(canonicalId, {
    limit: PAGE_SIZE,
  });
              ...
              {items.map((c) => (
                ...
                >
                  ...
                  <td className="py-3 pr-3 text-right font-mono text-surface-dark/80 dark:text-white/80">
                    {c.total_commits_in_project.toLocaleString()}
                  </td>
