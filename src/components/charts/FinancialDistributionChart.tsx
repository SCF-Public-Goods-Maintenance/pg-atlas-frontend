...
export function FinancialDistributionChart({ data, title }: FinancialDistributionChartProps) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:bg-white/5 dark:border-white/15">
      <h3 className="text-sm font-semibold text-[#0f0f21] dark:text-white mb-4">
        {title}
      </h3>
          ...
          return (
            <div key={point.label} className="flex-1 flex flex-col items-center gap-2 group">
              <div className="relative w-full flex justify-center items-end h-[120px]">
                {/* Tooltip */}
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-[#0f0f21] text-white text-[10px] py-1 px-2 rounded-md pointer-events-none whitespace-nowrap z-10">
              ...
              <span className="text-[10px] font-medium text-[#0f0f21]/50 dark:text-white/40 text-center">
                {point.label}
              </span>
            </div>
          )
